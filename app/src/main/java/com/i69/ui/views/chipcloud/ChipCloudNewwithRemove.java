...
public class ChipCloudNewwithRemove extends FlowLayout implements ChipListener {
    ...
    public ChipCloudNewwithRemove(Context context) {
        super(context);
        this.context = context;
        init();
    }

    public ChipCloudNewwithRemove(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
    ...
    public void setMode(Mode mode) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ChipNew chip = (ChipNew) getChildAt(i);
            chip.deselect();
    ...
    public void addChips(List<IdWithValue> chipList) {
        for (IdWithValue chipItem : chipList) {
            addChip(chipItem, false);
        }
    }

    public void addChip(IdWithValue chipItem, Boolean isTags) {
        String value;
        ...
        LinearLayout layout = (LinearLayout) inflater.inflate(R.layout.chip_looking_for_remove, null, false);

        AppCompatTextView tx = layout.findViewById(R.id.tx1);
        AppCompatTextView tx2 = layout.findViewById(R.id.tx2);

        ImageView ivC = layout.findViewById(R.id.ivClear);
        ...
        ivC.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                try {
                    removeView(layout);
                    chipsList.remove(chipItem);
                    BaseEditProfileFragment.listOfInterestedIn.remove(BaseEditProfileFragment.listOfInterestedIn.indexOf(chipItem.getId()));
...
//        ChipNew chip = new ChipNew.ChipBuilder()
//                .index(getChildCount())
//                .tag(chipItem.getId())
//                .label(value)
//                .typeface(typeface)
...
//                .build(context);
    ...
    public void removeChip(IdWithValue chipItem, Boolean isTags) {
        String value;
        if (UtilsKt.isCurrentLanguageFrench()) {
            value = chipItem.getValueFr();
        } else {
