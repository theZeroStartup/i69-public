...
import static com.i69.ui.views.rangeseekbar.SeekBar.INDICATOR_MODE_ALWAYS_SHOW;
...
//            }
        ...
        Log.e("MinimumValues", "" +leftValue);
        Log.e("minProgress", "" +minProgress);
        if (leftValue < minProgress) {
            throw new IllegalArgumentException("setValue() min < (preset min - offsetValue) . #min:" + leftValue + " #preset min:" + minProgress);
        }
        if (rightValue > maxProgress) {
            throw new IllegalArgumentException("setValue() max > (preset max - offsetValue) . #max:" + rightValue + " #preset max:" + maxProgress);
        }
