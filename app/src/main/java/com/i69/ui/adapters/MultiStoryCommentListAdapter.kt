...
import com.i69.ui.screens.main.moment.UserMultiStoryDetailFragment
import com.i69.ui.viewModels.CommentsModel
import com.i69.utils.loadCircleImage
import java.text.SimpleDateFormat
import java.util.*

class MultiStoryCommentListAdapter(
    private val ctx: Context,
    private val listener: UserMultiStoryDetailFragment,
    private var allusermoments: ArrayList<CommentsModel>?,
    private var momentAddCommentFragment: UserMultiStoryDetailFragment,
    private var isAdminOfMoments : Boolean,
    private var uid : String
) : RecyclerView.Adapter<MultiStoryCommentListAdapter.ViewHolder>() {
    private val formatter = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).apply {
        timeZone = TimeZone.getTimeZone("UTC")
    }

    override fun onBindViewHolder(holder: MultiStoryCommentListAdapter.ViewHolder, position: Int) {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): MultiStoryCommentListAdapter.ViewHolder =
        ViewHolder(
...
//                    }
                if(isAdminOfMoments || uid.equals(item.uid)){
//                if(isAdminOfMoments ){
...
//                        }
    ...
    interface ClickPerformListener {
        ...
        fun onUsernameClick(position: Int, models: CommentsModel)
