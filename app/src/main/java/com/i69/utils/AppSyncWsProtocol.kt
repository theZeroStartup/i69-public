...
class AppSyncWsProtocol(
    ...
    override suspend fun connectionInit() {
        val message = mutableMapOf<String, Any?>(
            "type" to "connection_init", "payload" to mapOf("content-type" to "application/json")
        )
    ...
    override fun <D : Operation.Data> startOperation(request: ApolloRequest<D>) {
        ...
        val data = NullableAnyAdapter.toJsonString(
        ...
        sendMessageMapText(
            mapOf(
                ...
                "payload" to mapOf(
                    "query" to DefaultHttpRequestComposer.composePayload(request)["query"],
                    "extensions" to mapOf(
    ...
    override fun handleServerMessage(messageMap: Map<String, Any?>) {
        ...
        return when (messageMap["type"]) {
            "data" -> listener.operationResponse(
                messageMap["id"] as String,
                messageMap["payload"] as Map<String, Any?>
            )
            "error" -> {
